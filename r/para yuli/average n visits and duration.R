...
n_visitas = readRDS('output/Historico_CONTROL_raw_VISITS_without_modelling/n_visitas_median_residuals_by_tattoo.rds') %>% 
  mutate(tatu = gsub("@.*", "", Ta)) %>% 
...
# Create the boxplot
boxplot(n_visitas$residuals_median ~ n_visitas$Control, 
        ylab = "N visits", 
        xlab = "Treatment",
        names = c("Stress", "Control"))  # Replace Name1 and Name2 with your desired labels

# Calculate the means
means <- tapply(n_visitas$residuals_median, n_visitas$Control, mean)
...
# Calculate the means
means <- tapply(animales_nvis$n_visitas, animales_nvis$Control, mean)
