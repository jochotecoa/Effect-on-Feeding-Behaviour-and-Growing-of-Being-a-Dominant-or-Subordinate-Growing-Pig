...
feed_data_1 = readRDS('output/ranking_diff_consumo_without_modelling/consumo_median_residuals_by_mixgroup.rds') %>% 
  ...
  mutate(ID = gsub('@.*', '', Ta)) %>% 
  merge.data.frame(hierarchy_data_12, by = 'ID') %>%
  group_by(Pen.x) %>% 
...
feed_data_12 = rbind.data.frame(feed_data_1, feed_data_2)
...
model <- lmer(Co_median ~ phase * hierarchy_change_value + (1 | ID), data = feed_data_12)
summary(model)$coefficients[4, 5] < 0.05
model <- lmer(Co_median_rel ~ phase * hierarchy_change_value + (1 | ID), data = feed_data_12)
summary(model)$coefficients[4, 5] < 0.05
...
nvis_total_data_12 = rbind.data.frame(nvis_total_data_1, nvis_total_data_2)
...
# Assuming your data frame is named 'pig_data'
model <- lmer(n_visitas_median ~ phase * hierarchy_change_value + (1 | ID), data = nvis_total_data_12)
summary(model)$coefficients[4, 5] < 0.05
model <- lmer(n_visitas_median_rel ~ phase * hierarchy_change_value + (1 | ID), data = nvis_total_data_12)
summary(model)$coefficients[4, 5] < 0.05
...
ti_total_data_12 = rbind.data.frame(ti_total_data_1, ti_total_data_2)
...
# Assuming your data frame is named 'pig_data'
model <- lmer(Ti_median ~ phase * hierarchy_change_value + (1 | ID), data = ti_total_data_12)
summary(model)$coefficients[4, 5] < 0.05
model <- lmer(Ti_median_rel ~ phase * hierarchy_change_value + (1 | ID), data = ti_total_data_12)
summary(model)$coefficients[4, 5] < 0.05
...
fr_total_data_12 = rbind.data.frame(fr_total_data_1, fr_total_data_2)
...
# Assuming your data frame is named 'pig_data'
model <- lmer(Fr_median ~ phase * hierarchy_change_value + (1 | ID), data = fr_total_data_12)
summary(model)$coefficients[4, 5] < 0.05
plot_model(model, type = "pred", terms = c("phase", "hierarchy_change_value"))

model <- lmer(Fr_median_rel ~ phase * hierarchy_change_value + (1 | ID), data = fr_total_data_12)
summary(model)$coefficients[4, 5] < 0.05
plot_model(model, type = "pred", terms = c("phase", "hierarchy_change_value"))
...
ti_median_data_12 = rbind.data.frame(ti_median_data_1, ti_median_data_2)
...
# Assuming your data frame is named 'pig_data'
model <- lmer(median_Ti_median ~ phase * hierarchy_change_value + (1 | ID), data = ti_median_data_12)
summary(model)$coefficients[4, 5] < 0.05
plot_model(model, type = "pred", terms = c("phase", "hierarchy_change_value"))

model <- lmer(median_Ti_median_rel ~ phase * hierarchy_change_value + (1 | ID), data = ti_median_data_12)
summary(model)$coefficients[4, 5] < 0.05
plot_model(model, type = "pred", terms = c("phase", "hierarchy_change_value"))
...
ti_range_data_12 = rbind.data.frame(ti_range_data_1, ti_range_data_2)

time_ranges = subset(colnames(ti_range_data_12), grepl("H", colnames(ti_range_data_12)))

for (time_range in time_ranges) {
  formulae = formula(paste0(time_range, " ~ phase * hierarchy_change_value + (1 | ID)"))
  
  model <- lmer(formulae, data = ti_range_data_12)
  
  model_summ = model %>% summary()
  
  if (any(model_summ$coefficients[4,5] < 0.05)) {
    # Plotting the model using sjPlot
    a = sjPlot::plot_model(model, show.values = TRUE)
    plot(a)
