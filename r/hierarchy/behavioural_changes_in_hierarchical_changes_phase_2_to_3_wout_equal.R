...
plot_model(model, show.values = T)

summary_pe_by_hierarchy <- feed_data_23 %>%
  group_by(hierarchy_change_value) %>%
  summarise(
...
dir.create("output/hierarchy/behavioural_changes_in_hierarchical_changes_phase_2_to_3_wout_equal/")

png(filename = "output/hierarchy/behavioural_changes_in_hierarchical_changes_phase_2_to_3_wout_equal/marginal_effects_plot_intake_vs_hierarchy_change_and_phase.png", 
    width = 1080*4, height = 720*4, res = 150*4)

model %>%
  plot_model(show.values = TRUE, type = "pred", 
             title = "Predicted values of Food Intake", 
             axis.title = c("Phase", "Food Intake (g)"), 
             legend.title = "Hierarchy Change",
             terms = c("phase", "hierarchy_change_value"))
