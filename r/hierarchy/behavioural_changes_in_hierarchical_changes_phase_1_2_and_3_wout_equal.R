...
hierarchy_data_123 = merge.data.frame(x = hierarchy_data_12, 
  ...
  mutate(hierarchy_change_value = paste(hierarchy_change_value.x, 
                                        ...
                                        sep = '_')) %>% 
  filter(grepl(x = hierarchy_change_value, "better_equal|worse_equal"))
...
ti_range_data_123 = rbind.data.frame(ti_range_data_1, ti_range_data_2, ti_range_data_3)
...
for (time_range in time_ranges) {
  ...
  if (any(model_summ$coefficients[5:6,5] < 0.05)) {
