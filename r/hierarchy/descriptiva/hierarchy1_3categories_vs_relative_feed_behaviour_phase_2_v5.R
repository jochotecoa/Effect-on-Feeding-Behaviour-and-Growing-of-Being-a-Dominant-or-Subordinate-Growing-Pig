...
dir.create("output/hierarchy/descriptiva/hierarchy1_3categories_vs_relative_feed_behaviour_phase_2_v5/")

png(filename = "output/hierarchy/descriptiva/hierarchy1_3categories_vs_relative_feed_behaviour_phase_2_v5/marginal_effects_plot_intake_vs_hierarchy.png", 
...
png(filename = "output/hierarchy/descriptiva/hierarchy1_3categories_vs_relative_feed_behaviour_phase_2_v5/marginal_effects_plot_relative_intake_vs_hierarchy.png", 
    width = 1080*4, height = 720*4, res = 150*4)
...
summary_pe_by_hierarchy <- feed_data_2 %>%
  group_by(hierarchy) %>%
  summarise(
    Mean = mean(Co_median, na.rm = TRUE),
    Median = median(Co_median, na.rm = TRUE),
    SE = sd(Co_median, na.rm = TRUE) / sqrt(n()),
    Min = min(Co_median, na.rm = TRUE),
    Max = max(Co_median, na.rm = TRUE),
    Count = n()
  )
...
for (variable in subset(colnames(ti_range_data_2), grepl('H', colnames(ti_range_data_2)))) {
  ...
  for (variable_i in c(variable, "time_rel")) {
    ...
    if (any(model_summ$coefficients[2,4] < 0.05)) {
