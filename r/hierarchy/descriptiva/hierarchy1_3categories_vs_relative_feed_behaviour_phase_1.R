...
co_lm = lm(Co_median ~ hierarchy + Pen, feed_data_1)
co_lm = lm(Co_median_rel ~ hierarchy, feed_data_1)
...
lm(n_visitas_median ~ hierarchy + Pen, nvis_total_data_1) %>% plot_model(show.values = T)
lm(n_visitas_median_rel ~ hierarchy, nvis_total_data_1) %>% plot_model(show.values = T)
...
lm(Ti_median ~ hierarchy + Pen, ti_total_data_1) %>% summary()
lm(Ti_median_rel ~ hierarchy, ti_total_data_1) %>% summary()
...
boxplot(Fr_median_rel ~ hierarchy, filter(fr_total_data_1, hierarchy != 'intermediate'))
lm(Fr_median ~ hierarchy + Pen, fr_total_data_1) %>% summary()
lm(Fr_median_rel ~ hierarchy, fr_total_data_1) %>% summary()
...
lm(median_Ti_median_rel ~ hierarchy, ti_median_data_1) %>% summary()


boxplot(median_Ti_median_rel ~ hierarchy, filter(ti_median_data_1, hierarchy != 'intermediate'))
...
lm(H6_H9 ~ hierarchy, ti_range_data_1) %>% sjPlot::plot_model(show.values = T)
