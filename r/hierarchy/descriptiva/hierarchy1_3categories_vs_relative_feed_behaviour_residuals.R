...
nvis_total_data_1 = readRDS('output/Historico_CONTROL_raw_VISITS/n_visitas_median_residuals_by_mixgroup.rds') %>%
  ...
  merge.data.frame(hierarchy_data, by = 'ID') %>% 
  group_by(Pen) %>% 
  mutate(submissive_mean = mean(ifelse(hierarchy == "submissive", residuals_nvis_median, NA), na.rm = TRUE)) %>% 
  mutate(n_visitas_median_rel = residuals_nvis_median/submissive_mean) %>% 
  ungroup()
