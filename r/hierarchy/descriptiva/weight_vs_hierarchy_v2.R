...
hierarchy_pesos_summary = rbind.data.frame(hierarchy_pesos_1_summary,
...
dir.create("output/hierarchy/descriptiva/weight_vs_hierarchy_v2/", recursive = T)

png(filename = "output/hierarchy/descriptiva/weight_vs_hierarchy_v2/barplot_weight_phase_hierarchy.png", 
...
p <- ggplot(hierarchy_pesos_summary, aes(x = phase, y = weight, fill = hierarchy)) + 
  ...
  geom_errorbar(aes(ymin = weight, ymax = weight + sd), width = 0.2, position = position_dodge(0.9)) + 
  labs(
...
bar_positions <- data.frame(
  ...
  y = c(146),  # Y-axis positions for the bars (adjust as needed)
...
p +
  ...
  annotate(
    ...
    x = c(2.70),
    xend = c(3.3),
    y = c(149),  # Adjust Y positions for bars
    yend = c(149),
