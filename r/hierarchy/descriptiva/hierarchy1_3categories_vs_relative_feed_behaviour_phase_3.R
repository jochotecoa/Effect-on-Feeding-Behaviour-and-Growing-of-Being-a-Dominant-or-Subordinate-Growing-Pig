...
co_lm = lm(Co_median_rel ~ hierarchy + Pen, feed_data_3)
co_lm %>% plot_model(show.values = T)
co_lm %>% plot_model(show.values = T, type = "pred")
...
lm(n_visitas_median_rel ~ hierarchy + Pen, nvis_total_data_3) %>% plot_model(show.values = T)
...
lm(Ti_median ~ hierarchy + Pen, ti_total_data_3) %>% plot_model(show.values = T, type = "pred")
lm(Ti_median_rel ~ hierarchy + Pen, ti_total_data_3) %>% plot_model(show.values = T, type = "pred")
...
#   boxplot(ti_range_data_3[[variable]] ~ ti_range_data_3$hierarchy, 
#           xlab = 'hierarchy', ylab = variable)
# }
# 
# for (variable in subset(colnames(ti_range_data_3), grepl('H', colnames(ti_range_data_3)))) {
#   a = ggstatsplot::ggbetweenstats(data = ti_range_data_3, x = hierarchy, 
#                               y = variable)
#   
# }
...
for (variable in subset(colnames(ti_range_data_3), grepl('H', colnames(ti_range_data_3)))) {
  ...
  if (shp_tst$p.value >= 0.05) {
    a = ggstatsplot::ggbetweenstats(data = ti_range_data_3_i, x = hierarchy, 
