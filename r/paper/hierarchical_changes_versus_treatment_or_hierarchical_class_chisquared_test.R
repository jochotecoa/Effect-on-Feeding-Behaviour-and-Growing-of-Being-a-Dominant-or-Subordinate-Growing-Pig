...
# }
# 
# chisq.test(all_possibs[c(1,5,9),])
# 
# chisq.test(all_possibs)
# # Calculate standardized residuals
# res <- chisq.test(all_possibs)$stdres
# 
# res[,2] %>% barplot(las = 2)
# 
...
# }
# 
# chisq.test(all_possibs[c(1, 5, 8), ])
# # Calculate standardized residuals
# res <- chisq.test(all_possibs[c(1, 5, 8), ])$stdres
# 
# res[,2] %>% barplot(las = 2)
# 
...
table(hierarchy_123$treatment.x.x, paste(hierarchy_123$hierarchy_change.x, hierarchy_123$hierarchy_change.y)) %>%
  t %>% as.matrix %>% .[c(1, 10, 18), ] %>%  chisq.test()
...
# Perform the chi-square test
chisq.test(all_possibs)
all_possibs[c(1, 10, 18),] %>% chisq.test()
...
all_possibs = data.frame(Observed = observed[,1], Random = c(1/2, 4, 1/2, 1/2, 4, 1/2))
