...
all_time_model = lmer(Weight ~ Acetitomaculum + Anaerobutyricum + 
       Coprococcus + Treponema + Floccifex + Treatment + Age + sexo + (1|Subject.ID.x) , data = genus_peso) 
...
# Scatter plot with color
  ...
  means_genus = genus_peso %>% 
    select(Acetitomaculum, Anaerobutyricum, Coprococcus, Floccifex, Treponema,
           FP_apprx, Treatment) %>% 
