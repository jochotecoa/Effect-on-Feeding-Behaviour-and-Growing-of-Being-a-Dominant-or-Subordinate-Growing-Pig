...
lm(Weight ~ Acetitomaculum + Anaerobutyricum + Coprococcus*sexo + Floccifex + 
     PeH17 + Treponema + Age, data = genus_peso) %>% 
  plot_model(type = "pred", terms = c("Coprococcus", "sexo [M]"))
genus_peso %>% filter(sexo == "M") %>% 
  lm(Weight ~ Acetitomaculum + Anaerobutyricum + Coprococcus + Floccifex + 
       PeH17 +Treponema + Age, data = .) %>% 
  plot_model(type = "pred", terms = "Coprococcus", colors = "blue")
...
time_model <- genus_peso %>% 
  select(where(is.numeric), Subject.ID.x) %>% 
  lmer(Weight ~ . - Subject.ID.x - day_of_life - studyid - library.Size - Corral - input - Treatment + 
         (1 | Subject.ID.x) + (1 | Corral), data = .)
