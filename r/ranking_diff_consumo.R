...
consumo_pesos$Co_rsdl = consumo_pesos$Co - consumo_pesos$pred

consumo_mediana = consumo_pesos %>% 
  group_by(Ta) %>% 
  summarise(Co_median = median(Co_rsdl, na.rm = T),
            Ta, Control) %>% 
  unique() %>% 
  # group_by(Ta) %>% 
  # mutate(Pe_i = min(Pe_app)) %>% 
...
consumo_mediano_by_group = consumo_pesos %>% 
  ...
  summarise(Co_median = median(Co_rsdl),
...
a = filter(consumo_pesos_2, Ta == 'M371@2022-03-24')
a$pred = predict(modelo_lm_control, a)
plot(a$edad, a$Co, main = unique(a$Ta), xlab = 'Edad', ylab = 'Consumo')
...
dir.create("output/ranking_diff_consumo")
...
diff_consumo_total %>% 
  ...
  filter(!Control) %>% 
  saveRDS("output/ranking_diff_consumo/diff_consumo_total.rds")
diff_consumo_ordered_first %>% 
  merge.data.frame(pesos_i, by.x = "Ta", by.y = "Ta") %>%
  merge.data.frame(pesos_f, by.x = "Ta", by.y = "Ta") %>% 
  mutate(diff_Pe = Pe_f - Pe_i) %>% 
  filter(!Control) %>% 
  saveRDS("output/ranking_diff_consumo/diff_consumo_ordered_first_mix.rds")
diff_consumo_ordered_second %>% 
  merge.data.frame(pesos_i, by.x = "Ta", by.y = "Ta") %>%
  merge.data.frame(pesos_f, by.x = "Ta", by.y = "Ta") %>% 
  mutate(diff_Pe = Pe_f - Pe_i) %>% 
  filter(!Control) %>% 
  saveRDS("output/ranking_diff_consumo/diff_consumo_ordered_second_mix.rds")

consumo_mediana %>% 
  saveRDS("output/ranking_diff_consumo/consumo_median_residuals_by_tattoo.rds")

consumo_mediano_by_group %>% 
  saveRDS("output/ranking_diff_consumo/consumo_median_residuals_by_mixgroup.rds")

diff_consumo %>% 
  merge.data.frame(pesos_i, by.x = "Ta", by.y = "Ta") %>%
  merge.data.frame(pesos_f, by.x = "Ta", by.y = "Ta") %>% 
  mutate(diff_Pe = Pe_f - Pe_i) %>% 
  saveRDS("output/ranking_diff_consumo/consumo_median_diff_mix_residuals.rds")