...
dir.create("output/ranking_diff_consumo_without_modelling")
...
diff_consumo_total %>% 
  ...
  filter(!Control) %>% 
  saveRDS("output/ranking_diff_consumo_without_modelling/diff_consumo_total.rds")
diff_consumo_ordered_first %>% 
  merge.data.frame(pesos_i, by.x = "Ta", by.y = "Ta") %>%
  merge.data.frame(pesos_f, by.x = "Ta", by.y = "Ta") %>% 
  mutate(diff_Pe = Pe_f - Pe_i) %>% 
  filter(!Control) %>% 
  saveRDS("output/ranking_diff_consumo_without_modelling/diff_consumo_ordered_first_mix.rds")
diff_consumo_ordered_second %>% 
  merge.data.frame(pesos_i, by.x = "Ta", by.y = "Ta") %>%
  merge.data.frame(pesos_f, by.x = "Ta", by.y = "Ta") %>% 
  mutate(diff_Pe = Pe_f - Pe_i) %>% 
  filter(!Control) %>% 
  saveRDS("output/ranking_diff_consumo_without_modelling/diff_consumo_ordered_second_mix.rds")

consumo_mediana %>% 
  saveRDS("output/ranking_diff_consumo_without_modelling/consumo_median_residuals_by_tattoo.rds")

consumo_mediano_by_group %>% 
  saveRDS("output/ranking_diff_consumo_without_modelling/consumo_median_residuals_by_mixgroup.rds")

diff_consumo %>% 
  merge.data.frame(pesos_i, by.x = "Ta", by.y = "Ta") %>%
  merge.data.frame(pesos_f, by.x = "Ta", by.y = "Ta") %>% 
  mutate(diff_Pe = Pe_f - Pe_i) %>% 
  saveRDS("output/ranking_diff_consumo_without_modelling/consumo_median_diff_mix_residuals.rds")